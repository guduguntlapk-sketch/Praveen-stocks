# Prave Stocks — simple BUY/HOLD/SELL using TA + news sentiment
...
# AI: finance sentiment (FinBERT)
from transformers import pipeline

st.set_page_config(page_title="Prave Stocks", page_icon="📈", layout="wide")
st.title("📈 Prave Stocks")
st.caption("Free demo: technicals + news sentiment → simple BUY / HOLD / SELL")
...
def load_news(ticker: str, limit: int = 15):
    """Use Yahoo news via yfinance. Falls back to empty list if not available."""
    try:
...
def load_sentiment_pipeline():
    # ProsusAI/finbert is a finance-tuned sentiment model
    return pipeline("text-classification", model="ProsusAI/finbert", truncation=True)

def score_headlines(df_news: pd.DataFrame, clf) -> pd.DataFrame:
    if df_news.empty:
        return df_news.assign(label=[], score=[], signed=[])
    preds = clf(df_news["title"].tolist())
...
def decide(ta_row: pd.Series, news_score: float):
    """Simple vote. Positive = buy pressure, negative = sell pressure."""
    score = 0
    reasons = []
    ...
    # Price vs SMA200 (trend filter)
    close = ta_row["Close"]
    sma200 = ta_row["SMA200"]
    if pd.notna(sma200):
        if close > sma200:
    ...
    try:
        ...
        if df0.empty:
            ...
            try:
                ...
                news_scored = score_headlines(news, clf)
                news_avg = float(news_scored["signed"].mean()) if not news_scored.empty else 0.0
            except Exception as e:
                st.warning(f"News sentiment offline ({e}). Using technicals only.")
                news_scored = news.assign(label=None, score=None, signed=None)
                news_avg = 0.0
